...
PanelWindow {
    ...
    // One PanelWindow instance is created per connected screen (see the
    // Variants wrapping StatusbarWindow in shell.qml), so each monitor gets
    // its own independent bar. Shared state - the merged settings and the
    // single "statusbar" IpcHandler - lives in the StatusbarState singleton
    // (StatusbarState.qml) instead, so these per-monitor windows don't
    // register duplicate IPC targets or each parse the settings file
    // independently.
    required property var modelData
    screen: modelData

    // The Hyprland monitor this window's screen corresponds to, used to tell
    // whether a StatusbarState IPC action (focus/expand/collapse) targets
    // this particular monitor's bar.
    readonly property HyprlandMonitor ownMonitor: root.screen ? Hyprland.monitorFor(root.screen) : null
    ...
    // Merged statusbar.json, owned by the shared StatusbarState singleton so
    // every per-monitor window instance reads the same settings instead of
    // each loading/parsing its own copy (see StatusbarState.qml).
    readonly property var settings: StatusbarState.settings
    ...
    // React to StatusbarState IPC actions (focus/expand/collapse) targeted at
    // whichever monitor currently has Hyprland's focus. Only the instance
    // whose ownMonitor matches acts, so SUPER + SHIFT + SPACE never grabs the
    // keyboard on more than one monitor's bar at once.
    Connections {
        target: StatusbarState
        function onPendingSerialChanged(): void {
            if (!root.ownMonitor || root.ownMonitor.id !== StatusbarState.pendingMonitorId)
                return
            if (StatusbarState.pendingAction === "focus") {
                root.barExpanded = true
                keyHandler.forceActiveFocus()
            } else if (StatusbarState.pendingAction === "expand") {
                root.barExpanded = !root.barExpanded
            } else if (StatusbarState.pendingAction === "collapse") {
                root.barExpanded = false
            }
        }
    }
